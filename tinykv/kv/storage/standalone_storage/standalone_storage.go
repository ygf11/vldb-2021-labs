...
func (s *StandAloneStorage) Write(ctx *kvrpcpb.Context, batch []storage.Modify) error {
	...
	for i := 0; i < len(batch); i++ {
		...
		case storage.Put:
			key := batch[i].Data.(storage.Put).Key
			value := batch[i].Data.(storage.Put).Value
			cf := batch[i].Data.(storage.Put).Cf
			...
			break
		case storage.Delete:
			key := batch[i].Data.(storage.Delete).Key
			cf := batch[i].Data.(storage.Delete).Cf
