...
func (c *Commit) PrepareWrites(txn *mvcc.MvccTxn) (interface{}, error) {
	...
	if txn.StartTS >= commitTs {
		return nil, fmt.Errorf("unexpected error for commit when txn.StartTs >= commitTs")
	}
...
func commitKey(key []byte, commitTs uint64, txn *mvcc.MvccTxn, response interface{}) (interface{}, error) {
	...
	if lock == nil || lock.Ts != txn.StartTS {
		...
		existingWrites, _, err := txn.CurrentWrite(key)
		if err != nil {
			return response, err
		}

		if existingWrites != nil && existingWrites.Kind != mvcc.WriteKindRollback {
			return response, nil
		}
